---
name: 5writing
description: "数学建模竞赛论文撰写阶段，支持 Typst 和 LaTeX 双引擎。根据 ANALYSIS_MODELING_REPORT.md、RESULTS_REPORT.md、results/figure_manifest.json 和 figures/final/ 选择模板、组织章节、插入冻结 run 的最终图，并按实际方法选择论文附录主要源程序。"
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, Agent, WebSearch, WebFetch
---

# 竞赛论文撰写（Typst / LaTeX）

本 skill 承接 `3coding-visual` 和 `4drawio`。前序阶段只提供真实冻结结果、图表 manifest、最终图片和记录文件；本阶段负责选择比赛模板和排版引擎、组织论文结构，并决定每张图表放入哪个章节。
...
论文中的所有数值与图表结论必须来自 `reports/RESULTS_REPORT.md`、current run 的权威结果和 `results/figure_manifest.json`。最终论文图片只从 `figures/final/` 选择，不得使用 `figures/preview/`，也不得编造、估算或改变四舍五入口径。
...
在写正文各节之前，根据 `results/figure_manifest.json`、`figures/final/`、`reports/RESULTS_REPORT.md`，以及 `reports/DRAWIO_REPORT.md`（如果存在）构建图表规划。对每张图核对 `figure_id`、`display_name`、`caption`、`claim`、`problem`、`script_path`、`input_result_files`、`input_fields`、`output_png` 和 `output_pdf_or_svg`，并确认输入属于 current run：
...
图片路径相对于写入该图片的文件：写在 `paper/main.typ` 或 `paper/main.tex` 中通常用 `../figures/final/xxx.pdf`，写在 `paper/sections/*.typ` 或 `paper/sections/*.tex` 中通常用 `../../figures/final/xxx.pdf`。不要到 `code/plots/`、`figures/preview/` 或各问题结果目录寻找论文图片。

**Typst 引擎**图片插入：

```typst
#figure(
  image("../../figures/final/Q1_图01_预测误差分布.pdf", width: 85%),
  caption: [问题一预测误差分布],
)
```
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../figures/final/Q1_图01_预测误差分布.pdf}
  \caption{问题一预测误差分布}
  \label{fig:q1_error}
\end{figure}
```
...
3_assumptions.tex
4_symbols.tex
5_modeling.tex       - 第五章“模型的建立与求解”入口
5_1_problem1.tex     - 问题一的模型建立与求解
5_2_problem2.tex     - 问题二的模型建立与求解
5_3_problem3.tex     - 问题三的模型建立与求解
5_4_problem4.tex     - 问题四的模型建立与求解
6_optional_model_analysis.tex - 默认关闭的统一模型分析与检验
6_evaluation.tex     - 模型的评价、改进与推广
ai_declaration.tex   - 参考文献前的AI工具使用声明
A_appendix.tex       - 支撑材料、环境、完整代码与补充结果
support/ai-tool-usage-details/main.tex - 独立AI使用详情PDF模板
```

国赛 B 题 LaTeX 模板默认按四问组织。第五章只建立一个 `\section{模型的建立与求解}`，每个问题使用一个 `\subsection`，并依次包含以下五个 `\subsubsection`：
...
- 不固定设置“数据预处理”小节；只有模型确实需要时，才在建模思路或求解步骤中说明处理方法及理由。
- “模型的建立”末尾用不进入目录的“模型汇总”集中给出最终模型、约束和变量范围。
- “模型的求解”使用连续的 `Step 1`、`Step 2`……描述真实求解流程，步骤数量按算法调整。
- “模型检验与灵敏度分析”只选与本问模型匹配的 1～2 项；没有现实不确定参数时允许不做灵敏度分析。
- 模型检验必须验证数学关系、拟合或预测效果、排序稳定性、约束可行性、基准改进、边界条件等论文结论，不能把数组越界、维度一致或程序成功运行写成模型检验。
- 所有检验指标、扰动结果和图表都必须来自真实求解记录。若现有结果不足，返回 `3coding-visual` 真实补算并冻结结果，禁止补写或编造实验。
- 全文使用同一模型或同类模型时，可启用 `6_optional_model_analysis.tex` 并删除各问重复检验；模型差异较大时保留分问检验；只有主要模型需要分析时只保留对应小节。三种布局不得混用。
- CUMCM 2026 终稿不生成目录，摘要页页码从 1 开始。AI工具使用声明必须位于参考文献之前且只保留真实情形；经本AI工作流生成论文时不得声明“未使用AI”。
- 使用AI时，同时从 `support/ai-tool-usage-details/main.tex` 生成 `AI工具使用详情.pdf`，如实记录工具、用途、提示过程、采纳修改和人工核验。
- 附录必须由 `delivery/delivery_manifest.json` 和真实交付清单生成。附录 C 按实际问题数选择能体现主要模型、核心算法和重要验证方法的完整必要源程序，不机械复制整个工程。仅做参数转发的 main、简单字段检查和纯绘图脚本默认不进入论文附录；完整绘图脚本仍保留在电子支撑材料 `code/plots/`。不得保留固定算法示例、伪代码、空框架或不存在的环境与文件。
...
AI工具及其输出不作为参考文献；其使用情况写入论文参考文献之前的声明，并在支撑材料中单独说明。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../figures/final/Q1_图01_结果图.pdf}
  \caption{图注}
  \label{fig:q1}
\end{figure}
