...
承接 `analysis-modeling`，建立“数据 → 模型 → 求解 → 验证 → run 冻结 → 选图 → 独立绘图 → 写作交接 → 支撑材料导出”的可追溯链路。
...
- CUMCM B 题国赛论文使用的 [paper-writing 结果交接合同](references/paper-writing-result-handoff.md)。
...
`paper-writing` 只消费 `figures/final/` 的入选图片和 manifest，不到各问题目录猜测文件。
