...
在每个子问题完成正式求解、验证并形成 promoted run 后使用本合同。它规定 `coding-visual` 如何调用项目内的 `$model-figure-selector`，以及如何把论文 claim、冻结结果、独立绘图脚本和最终图片连接起来。
...
| 图表 | 归属 |
|---|---|
| 真实数据分布、预测诊断、收敛、Pareto、敏感性、约束检查 | `coding-visual` |
| 由节点/边/坐标/权重/求解方案生成的网络、路径、布局 | `coding-visual` |
| 技术路线、通用算法流程、数据处理流程、指标层次、纯概念结构 | `4drawio` |
| 需要精确数据但被包装成“示意图”的图 | 仍归 `coding-visual`，不得改成交给生成式图片 |
