...
本 skill 承接 `coding-visual` 和 `4drawio`。前序阶段只提供真实冻结结果、图表 manifest、最终图片和记录文件；本阶段负责选择比赛模板和排版引擎、组织论文结构，并决定每张图表放入哪个章节。
...
- 所有检验指标、扰动结果和图表都必须来自真实求解记录。若现有结果不足，返回 `coding-visual` 真实补算并冻结结果，禁止补写或编造实验。
